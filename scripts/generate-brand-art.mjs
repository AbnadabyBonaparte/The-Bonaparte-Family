/**
 * Pipeline de arte de marca — The Bonaparte Family.
 *
 * Gera as peças cinematográficas do portal e salva em client/public/brand/.
 * Lê as chaves do ambiente (presentes no Vercel; ausentes localmente → PULA sem erro).
 * O hero referencia "/brand/hero.jpg" por caminho público: se o arquivo não existe,
 * o hero cai graciosamente no fallback (foto real + gradiente) — o build NUNCA quebra.
 *
 * Uso: node scripts/generate-brand-art.mjs   (npm run art:generate)
 */
...
const OUT = process.env.ART_OUT || "client/public/brand";
...
async function viaIdeogram(prompt, aspect) {
  const ar = aspect === "16:9" ? "ASPECT_16_9" : aspect === "1:1" ? "ASPECT_1_1" : aspect === "3:2" ? "ASPECT_3_2" : "ASPECT_16_9";
...
async function run() {
  ...
  for (const p of cfg.pieces) {
    try {
      let url;
      if (FAL) {
        try { url = await viaFal(p.prompt, p.aspect); }
        catch (e) { console.warn("[art] Fal falhou, tentando Ideogram:", e.message); }
      }
